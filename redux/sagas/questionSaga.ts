...
import { questionEndpoints } from '@/common/endpoints/QuestionEndpoint';
...
function* handleDeleteQuestion(action: ReturnType<typeof deleteQuestionRequest>) {
    try {
        const response: UnifiedResponse<string> = yield call(
            apiCall,
            questionEndpoints.deleteById(action.payload),
            'DELETE',
            getAuthenticatedHeader()
        );
...
function* handleFetchQuestions(action: ReturnType<typeof fetchQuestionsRequest>) {
    try {
        const path = questionEndpoints.filters(action.payload);
        const response: UnifiedResponse<PageResponse<Question>> = yield call(
            apiCall,
            path,
            'GET',
            getAuthenticatedHeader()
...
function* handleUpdateQuestion(action: ReturnType<typeof updateQuestionRequest>) {
    try {
        const response: UnifiedResponse<string> = yield call(
            apiCall,
            questionEndpoints.updateById(action.payload.id),
            'PUT',
            getAuthenticatedHeader(),
            action.payload
        );
